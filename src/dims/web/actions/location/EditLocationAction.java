...
public class EditLocationAction extends Action  {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    logger.info("Entering EditLocationAction***");
    
    try {
      session = request.getSession(false);
      locationNewEditForm = (LocationNewEditForm)form;
      DataSource ds = getDataSource(request,"DIMS");
      Operations.editLocation(ds,locationNewEditForm);
      msg = new ActionMessage("msg.edit.location",locationNewEditForm.getTxtLocationID());
      messages.add("newLocation",msg);
      
    }catch (SQLException sqle) {
      logger.error("An error occured in EditLocationAction");
      logger.error(sqle.toString());
      if( sqle.getSQLState().equals(ErrorConstants.UNIQUE.getErrorCode()) ){
        msg=new ActionMessage("msg.Unique.LocationId",locationNewEditForm.getTxtLocationID());
    ...
    logger.info("Exiting EditLocationAction***");
