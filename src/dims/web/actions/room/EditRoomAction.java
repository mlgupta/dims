...
public class EditRoomAction extends Action  {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    logger.info("Entering EditRoomAction***");
    
    try {
      session = request.getSession(false);
      roomNewEditForm = (RoomNewEditForm)form;
      DataSource ds = getDataSource(request,"DIMS");
      Operations.editRoom(ds,roomNewEditForm);
      msg = new ActionMessage("msg.edit.roomNumber",roomNewEditForm.getTxtRoomNumber());
      messages.add("editRoom",msg);
      
    }catch (SQLException sqle) {
      logger.error("An error occured in EditRoomAction");
    ...
    logger.info("Exiting EditRoomAction***");
