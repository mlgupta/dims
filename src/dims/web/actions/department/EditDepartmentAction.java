...
public class EditDepartmentAction extends Action  {
  ...
  public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
    ...
    logger.info("Entering EditDepartmentAction***");
    
    try {
      session = request.getSession(false);
      departmentNewEditForm = (DepartmentNewEditForm)form;
      DataSource ds = getDataSource(request,"DIMS");
      Operations.editDepartment(ds,departmentNewEditForm);
      msg = new ActionMessage("msg.edit.dept",departmentNewEditForm.getTxtDepartmentId());
      messages.add("newDept",msg);
      
    }catch (SQLException sqle) {
      logger.error("An error occured in EditDepartmentAction");
    ...
    logger.info("Exiting EditDepartmentAction***");
